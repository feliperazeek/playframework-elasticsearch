...
A few changes were introduced on 0.0.5 to facilitate searching capabilities. 
We are adding a thin layer between the client application and Elastic Search to reduce the amount of complexity required to do searchin as well, not just indexing.

			FieldQueryBuilder builder = new FieldQueryBuilder ("fieldName", value);
			SearchResults results = ElasticSearch.search(builder);
			

SearchResults a very simple POJO with two properties for now, totalCount and list of objects.
			

				        
See ElasticSearch.com for more information.
